	...
	def object_cb(self, msg):
		...
		if self.Finish:
			...
			for i in range(self.obj_list.size):
				if (self.obj_list.list[i].position.y < 0):
			...
			if tfSuccess:
				...
				if (entrance_del>1 or exit_del>1):
					entrance.x = self.last_entrance.x
					entrance.y = self.last_entrance.y
					exit.x = self.last_exit.x
					exit.y = self.last_exit.y
				...
				#Calculate waypoints
				entrance_wp.x = entrance.x - (exit.x - entrance.x) / 10
				entrance_wp.y = entrance.y - (exit.y - entrance.y) / 10
				exit_wp.x = exit.x + (exit.x - entrance.x) / 10
				exit_wp.y = exit.y + (exit.y - entrance.y) / 10
