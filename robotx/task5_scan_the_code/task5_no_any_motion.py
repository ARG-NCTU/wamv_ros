...
class task5_node():
	...
	def process(self, event):
		...
		if not self.start or self.objlist is None or self.objlist.size is 0 or self.wamv_pose is None:
			return
		...
		for i in range(0, self.objlist.size):
			...
			if "light_buoy" in self.objlist.list[i].type:
