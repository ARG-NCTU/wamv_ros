	...
	def update(self, x_pos, y_pos, x_vel, y_vel, yaw):
		...
		if station_keep_flag == 0:
			...
			if pos_error > (3+1.5):
				...
				self.vel_output = linear_vel_const*0.1
			return 0
		...
		if (vel_delta_time >= self.sample_time):
			...
			if pos_error < 3:
				self.vel_output = 0				
				if np.sqrt(x_vel*x_vel+y_vel*y_vel) < 0.15:
					stable = 1
...
class ang_PID:
	...
	def __init__(self, P=0.6, I=0.01, D=0.4):
		global lock_aux_pointx
		self.wait_flag = 0
		self.wait_start = 0
		self.Kp = P
		self.Ki = I
...
def clear_waypoints_handler(req):
	global new_waypoints, waypoint_index, start_flag, station_keep_flag, lock, lock_aux_pointx
...
if __name__ == "__main__":	
	...
	rospy.Subscriber("/wamv/odometry/filtered", Odometry, cb_nav)
	
	pub_vel = rospy.Publisher("cmd_drive", roboteq_drive, queue_size = 20)
	...
	while not rospy.is_shutdown():
		...
		if start_flag == 0:
			...
			msg = roboteq_drive()
			msg.left = (out + ang_left)*1000
			msg.right = (out + ang_right)*1000
			pub_vel.publish(msg)
