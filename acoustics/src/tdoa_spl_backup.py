...
class TDOA_spl(object):
    ...
    def Iterate(self, event):
        ...
        if (self.waitForWindowFullEnd - self.waitForWindowFullStart) >= self.window_time and self.startIndex == True and self.getDataIndex == True:
            ...
            ch1filter = self.tool.butter_bandpass_filter(self.m1.receiveData, self.fs, self.highPass, self.lowPass)
            ch2filter = self.tool.butter_bandpass_filter(self.m2.receiveData, self.fs, self.highPass, self.lowPass)
            ...
            #c1max = np.amax(ch1filter)
            #c2max = np.amax(ch2filter)
            c1BiggerIndex, _ = find_peaks(ch1filter, height=c1Threshold, distance=2)
            c2BiggerIndex, _ = find_peaks(ch2filter, height=c1Threshold, distance=2)
            #c1BiggerIndex = np.where(ch1filter > self.voltThreshold)
            #c2BiggerIndex = np.where(ch2filter > self.voltThreshold)
            if c1BiggerIndex.size != 0 and c2BiggerIndex.size != 0 :
                ...
                index, self.guessAngle = self.tool.twoMicsAngle(self.m1_m2_dl, self.m1.rt, self.m2.rt, self.c)
                if self.guessAngle != 400:
                    print("C1 receive time = ", self.m1.rt)
                    print("C2 receive time = ", self.m2.rt)
                    print("guess angle = ", self.guessAngle)
            ...
            print("c1max = ", c1Threshold)
            print("c2max = ", c2Threshold)
...
if __name__ == '__main__':
    rospy.init_node('tdoa_spl', anonymous=False)
    tdoa_spl = TDOA_spl()
