...
class TDOA_spl(object):
    ...
    def OnStartUp(self):
        ...
        if rospy.has_param('~windowLen'):
            self.window_time = rospy.get_param('~windowLen')
    ...
    def serviceCallback_backup(self, request):
        while(True):
            if self.flag_getdata == True:
                ...
                if(np.mean(ch1prefilter)*self.thresholdTimeofMean <= np.amax(ch1prefilter) and np.mean(ch2prefilter)*self.thresholdTimeofMean <= np.amax(ch2prefilter)):
                    ...
                    if c1BiggerIndex.size != 0 and c2BiggerIndex.size != 0:
                        ...
                        self.guessAngle = self.findDiffT(np.absolute(c1), np.absolute(c2), self.m1_m2_dl / self.c)
                        if self.guessAngle == 400:
                            ...
                            rospy.loginfo("Angle is [%f]." %(self.guessAngle))
    ...
    def findDiffT(self, inputData1, inputData2, maxDt):
        count = 2.0
        while(1):
            BiggerIndex1, _ = find_peaks(inputData1, height=np.mean(inputData1)*count, distance=20)
            BiggerIndex2, _ = find_peaks(inputData2, height=np.mean(inputData2)*count, distance=20)
