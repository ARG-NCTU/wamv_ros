...
class GoalNav(object):
    def __init__(self):
        ...
        self.action_scale = {'linear': rospy.get_param(
            '~linear_scale', 0.4), 'angular': rospy.get_param("~angular_scale", 0.435)}
        ...
        model_path = os.path.join(my_dir, "../model/vel-omega-v3/snapshots/policy")
        ...
        self.sub_imu = rospy.Subscriber(
            "imu_in",  Imu, self.cb_imu, queue_size=1)
    ...
    def cb_odom(self, msg):
        ...
        if self.last_time is not None and self.last_pos is not None:
            ...
            # self.omega = (yaw-self.last_yaw)/self.time_diff
        ...
        if self.velocity_track is None:
            self.velocity_track = np.tile(self.velocity, (self.pos_n, 1))
        else:
            self.velocity_track[:-1] = self.velocity_track[1:]
            self.velocity_track[-1] = self.velocity
    ...
    def cb_imu(self, msg):
        ...
        self.omega = np.array([self.omega])
        # print("ang_vel: ", self.omega)
        if self.omega_track is None:
            self.omega_track = np.tile(self.omega, (self.pos_n, 1))
        else:
            self.omega_track[:-1] = self.omega_track[1:]
            self.omega_track[-1] = self.omega
    ...
    def inference(self, event):
        ...
        w_vel = self.omega_track.reshape(-1)
        state = np.append(laser, track)
        state = np.append(state, vel)
        state = np.append(state, w_vel)
